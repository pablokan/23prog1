...
def cantidadClientesXlocalidad(ciudad):
    c = 0
    for persona in datos:
        if ciudad in persona:
            c += 1
    print(f'La cantidad de clientes de {ciudad} es {c}')

cantidadClientesXlocalidad('La Carlota')
# localidad = input('Ingrese localidad: ')
# cantidadClientesXlocalidad(localidad)

def deudaAcumGranDeudores(deudaPiso):
    ac = 0
    for persona in datos:
        _, _, deuda, _ = persona.split('#')
        if int(deuda) > deudaPiso:
            ac += int(deuda)
    print(f'El total de deuda acumulada de los clientes que deben más de {deudaPiso} pesos es {ac}.')

deudaAcumGranDeudores(90000)

def apellidosJovenes(dniPiso):
    for persona in datos:
        dni, apYno, _, _ = persona.split('#')
        if int(dni) > dniPiso:
            print(apYno.split()[1])

apellidosJovenes(40_000_000)
