...
for persona in inversores:
    perso = persona.split(",")
    nombres.append(perso[1])
    ciudades.append(perso[4])
    sexos.append(perso[3])
pregCiudad= input("ingrese ciudad: ")
contar= -1
for ciudad in ciudades:
    contar = contar + 1
    if ciudad == pregCiudad:
        print("el nombre de la persona que vive en", pregCiudad,"es: ", nombres[contar])
...
for dividir in inversiones:
    div = dividir.split(" - Fecha:")
    fechas.append(div[1])
    divMontos = div[0].split("$")
    montos.append(divMontos[1])
for anio in fechas:
    a = anio.split("/") 
    anios.append(a[2])  
    mes.append(a[1])
...
for i in range(len(inversores)):
    ...
    if sexo == "Male":
        ...
        anio = int(fecha_inversion.split("/")[2])
        if anio == 2021 and mes >= 7:
         total_inversiones = total_inversiones + float(montos[i])
        
        
print("El total de fondos invertidos por varones en el segundo semestre del 2021 fue de : $", total_inversiones)
