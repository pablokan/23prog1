...
print("La mayor inversión en el 2022 fue de $", mayorInversion)
...
for inversion in inversiones:
    if '2021' in inversion:
        fecha = inversion.split('Fecha:')[1].split('/')
        mes = fecha[1]
        if 7 <= int(mes) <= 12:
            for inversor in inversores:
                    genero = inversor.split(',')[3]
                    if genero == 'Male':
                        tInverMasculinas += 1

print("El total de fondos invertidos por varones en el segundo semestre del 2021 fue de:", tInverMasculinas)