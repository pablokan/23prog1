...
buscar=input("Ingresar el nombre de la ciudad:")

for inversores in inversores:
    datos=inversores.split(",")
    ciudades=datos[4].split()[-1]
    nombre=datos[1].split()
    if buscar == ciudades:
        ciudad.append(nombre)
print("el nombre de la persona es:",ciudad)
...
for inversiones in inversiones:
    info=inversiones.split(":")
    sueldos=info[1].split("-")
    fecha=info[2].split("/")
    año=int(fecha[2022])
    if año:
        inversion_2022.append(sueldos)

print("La mayor inversion realizada en 2022 fue de:",inversion_2022)
