...
busquedaInversor = input('Ingrese la ciudad del inversor: ')
for i in range(len(inversores)):
    datosInversor = inversores[i]
    datosInversor = datosInversor.split(',')
    if datosInversor[4] == busquedaInversor:
        nombreInversor = datosInversor[1]
        ciudadInversor = datosInversor[4]
print('El nombre de la persona que vive en', ciudadInversor, 'es:', nombreInversor)

# 2) Decir el monto de la mayor inversión realizada en el 2022.

inversiones2022 = []
for i in range(len(inversiones)):
    inversionFecha = inversiones[i]
    inversionFecha = inversionFecha.split(' - ')
    inversion = inversionFecha[0]
    inversion = inversion.split(': $')
    monto = inversion[1]
    fecha = inversionFecha[1]
    fecha = fecha.split('/')
    año = fecha[2]
    if año == '2022':
        inversiones2022.append(monto)
print('La mayor inversion del año 2022 fue de:', max(inversiones2022))
...
for i in range(len(inversores)):
    ...
    if sexo == 'Male':
        if año == '2021':
            if mes > 6:
                inversionesHombres.append(monto)
print('El total de fondos invertidos por varones en el segundo semestre del 2021 fue de', sum(inversionesHombres))

# Alumno: Maldonado Valentin. Comision: A