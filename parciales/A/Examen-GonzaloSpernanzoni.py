...
inversores_por_ciudad = []

for inversor in inversores:
    datos_inversor = inversor.split(',')
    nombre = datos_inversor[1]
    ciudad = datos_inversor[4]
    
    inversores_por_ciudad.append((ciudad, nombre))

ciudadBuscada = input("Ingrese el nombre de la ciudad: ")

print("Inversores en", ciudadBuscada + ":")
for inversor in inversores_por_ciudad:
    if inversor[0] == ciudadBuscada:
        print( inversor[1]) """
...
for inversion in inversiones:
    montoString = inversion.split(' ')[1][1:]  

    parteEntera, parteDecimal = montoString.split('.')
    
    monto = int(parteEntera) + int(parteDecimal) / 100
    fecha = inversion.split(' ')[-1].split(':')[1]
    año = fecha.split('/')[2]
    
    if año == '2022' and monto > mayorInversion:
        mayorInversion = monto

print("El monto de la mayor inversión en el 2022 es:", mayorInversion) """
...
for i in range(len(inversores)):
    ...
    if "Male" in inversor:
        ...
        if año == 2021 and 7 <= mes <= 12: 

            montoString = inversion.split(' - ')[0].split('$')[1]
            montoYdecimales = montoString.split('.')
            montoInt = int(montoYdecimales[0]) * 100 + int(montoYdecimales[1])
            totalInversiones += montoInt

totalInversion = (totalInversiones // 100)+(totalInversiones % 100)

print("El total de inversiones realizadas durante el segundo semestre del año 2021 por inversores de sexo masculino es:",totalInversion)
