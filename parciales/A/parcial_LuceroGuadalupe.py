...
print("Actividad 1: ")
ciudadBuscada=input("Ingrese nombre de ciudad: ")
nombre = []

for inversores in inversores: 
    lista=inversores.split(",")
    nombres=lista[4].split()[-1]
    nombreInversor= lista[1].split()
    if ciudadBuscada==nombres:
        nombre.append(nombreInversor)
print("El nombre de la persona que vive en", ciudadBuscada, "es", nombre)
...
añoPedido=2022
inv2022=[]
for inversiones in inversiones:
    inv=inversiones.split(":")
    monto=inv[1].split("-")
    fecha=inv[2].split("/")
    año=int(fecha[2])
    if añoPedido==año:
        inv2022.append(monto)

print("La mayor inversión realizada en 2022 fue de:",inv2022)
...
dinero=[]
for inversores in inversiones:
    info=inversores.split(",")
    sexo=info[3].split()[-1]
    dato=inversiones.split(":")
    din=dato[1].split("-")
    fecha=dato[2].split("/")
    año=fecha[2]
    mes=fecha[1]
    if año==2021 and mes>=7 and sexo=="Male":
        dinero.append(dinero)
print("El total de fondos invertidos por varones en el segundo semestre del 2021 fue de", dinero)