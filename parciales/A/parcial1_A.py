...
for persona in inversores:
    if 'Dongping' in persona:
        print('El nombre de la persona que vive en Dongping es:', persona.split(',')[1])

mayorInversion2022 = 0
for inversion in inversiones:
    if '/2022' in inversion:
        guita = float(inversion[8:17])
        if guita > mayorInversion2022:
            mayorInversion2022 = guita
print('La mayor inversión del año 2022 fue de:', mayorInversion2022)

totVaronesSem2_2021 = 0
for i in range(len(inversores)):
    a = int(inversiones[i][-4:])
    m = int(inversiones[i][-7:-5])
    if 'Male' in inversores[i] and a == 2021 and m > 6:
        guita = float(inversiones[i][8:17])
        totVaronesSem2_2021 += guita
print('Total de fondos invertidos por varones en el segundo semestre del 2021:', totVaronesSem2_2021)
