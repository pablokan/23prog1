...
for inversor in inversores:
    datoSolicitado = inversor.split(",")
    nombre = datoSolicitado[1]
    ciudad = datoSolicitado[-1]
    nombres.append(nombre)
    ciudades.append(ciudad)

ciudadSolicitada = input("Ingrese una ciudad:")
for i in range(len(ciudades)):
    if ciudades[i] == ciudadSolicitada:
        print("El nombre de la persona que vive en", ciudadSolicitada, "es:", nombres[i])

"""montos = []        

for monto in inversiones:
    montoMayor = monto.split(":" and "-")
    montito = montoMayor[7]
    montos.append(montito)
    print(montos)"""
