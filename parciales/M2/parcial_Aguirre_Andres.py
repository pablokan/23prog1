...
inversiones_2021 = [inversion for inversion in inversiones if '2021' in inversion.split('-')[1]]
total_inversiones_2021 = sum(float(inversion.split('$')[1].split()[0]) for inversion in inversiones_2021)

print("Total de inversiones realizadas en el año 2021:", total_inversiones_2021)
...
# 4) Obtener el correo electrónico de Fionna
correo_fionna = None
for inversor in inversores:
    datos = inversor.split(",")
    if datos[1].lower() == "fionna":
        correo_fionna = datos[2]
        

if correo_fionna:
    print(f"El correo electrónico de Fionna es: {correo_fionna}")
else:
    print("No se encontró el correo electrónico de Fionna")
