...
nombreABuscar = input("Ingrese un nombre:  ")
for i in range(len(inversores)):
    sep = inversores[i].split(",")
    if nombreABuscar == sep[1]:
        print("El mail de", nombreABuscar, "es:", sep[2])

montos = []
total = 0
for a in range(len(inversiones)):
    sep2 = inversiones[a].split(" ")
    año = sep2[3][-4:]
    monto = sep2[1][1:]
    if año == "2021":
        montos.append(monto)
for plata in montos:
    plata = float(plata)
    total += plata
print("Total de fondos del año 2021:",total)

inversionesM = []
contador = 0
for r in range(len(inversores)):
    sep3 = inversores[r].split(",")
    if sep3[3] == "Female": 
        inversionesM.append(inversiones[r])
for p in range(len(inversionesM)):
    sep4 = inversionesM[p].split(" ")
    año = sep4[3][-4:]
    mes = sep4[3][-7:-5]
    if año == "2022" and mes <= "06":
        contador += 1
print("Cantidad de inversiones de mujeres en el primer semestre del 2022:", contador)