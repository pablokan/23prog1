...
nombreBuscar = input("Ingresar nombre: ")
for i in range (len(inversores)):
    sep = inversores[i].split(",")
    if nombreBuscar == sep[1]:
        print("El mail de", nombreBuscar, "es", sep[2])
...
for i in range (len(inversiones)):
    sep2 = inversiones[i].split("-")
    fecha = sep2[1]
    fechaAño = fecha[-4:]
    monto = sep2[0]
    montoExacto = float(monto[8:])
   

    if año == fechaAño:
        montos.append(montoExacto)
  
for plata in montos:
    plata = float(plata)
    montoTotal += plata
    
print("Total de fondos del año 2021: ","$",montoTotal)
...
for i in range (len(inversores)):
    sep3 = inversores[i].split(",")
    sexo = sep3[3]
    sep4 = inversiones[i].split("-")
    fecha = sep4[1]
    fechaNum = fecha[-10:]
    fechaMes = int(fechaNum[3:5])
    
    if f == sexo:
       if trimestre >= fechaMes:
           total += 1
