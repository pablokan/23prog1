...
n = [ ]
mails= [ ]
nombre_del_inversor = input("ingresar nombre: ")
for inversor in inversores:
     signo = inversor.split(',')
     nombre = signo[1]
     correo = signo[2]
     mails.append(correo)
     n.append(nombre)
...
for i in range(len(n)):
    if nombre_del_inversor.lower() == n[i].lower():
        print("Correo del inversor buscado:", mails[i])
"""""
inversiones_2021 = [ ]
total = 0

for inversion in inversiones:
    signo_monto = inversion.split('Monto: $')
    
    if len(signo_monto) > 1:
        monto = float(signo_monto[1])
        total += monto
        
    signo_año = inversion.split('/')
    año = signo_año[2]
    
    if año == "2021":
        inversiones_2021.append(inversion)
