...
for dato in datos:
    dato_lista = dato.split('#')
    deuda = int(dato_lista[2])
    if deuda > mayor_deuda:
        mayor_deuda = deuda
        apellido_m_deudor = dato_lista[1].split()[1]

print('El apellido del mayor deudor es: ', apellido_m_deudor)
...
for dato in datos:
    lista = dato.split('#')
    if lista[3] == 'Río Cuarto':
        deuda_rio_cuarto += int(lista[2])

print('El total acumulado de los Riocuartenses es de', deuda_rio_cuarto)
...
for dato in datos:
    lista_2 = dato.split('#')
    if lista_2[0].startswith('2'):
        nombres = lista_2[1].split()[1]
        print(''.join(nombres))
