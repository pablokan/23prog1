...
for dato in datos:
    splitted = dato.split("#")
    if int(splitted[2]) > mayorDeuda:
        mayorDeuda = int(splitted[2])
        nombreCompleto = splitted[1]
        apellido = nombreCompleto.split(" ")[1]
print(f'El apellido del mayor deudor es: {apellido}')

# res 2
sum = 0
for dato in datos:
    if "Río Cuarto" in dato:
        sum += int(dato.split("#")[2])
print(f'El total de deuda acumulada de los riocuartenses es de ${sum}')
