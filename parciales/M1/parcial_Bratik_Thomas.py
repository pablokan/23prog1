...
print("Act1:")
totalClientescarlota = 0
for clientes in datos:
    buscadorCarlota = clientes.split("#")
    ubicacionCliente = buscadorCarlota[3]
    if ubicacionCliente == "La Carlota":
        totalClientescarlota+=1
    
print(totalClientescarlota)
...
for cliente in datos:
    ubiDeuda = cliente.split("#")
    deuda = int(ubiDeuda[2])  
    if deuda > 90_000:  
        totalDeuda += deuda
print("El total de la deuda de los clientes con deuda mayor a 90,000 es de:", totalDeuda)


print("Act3:")

for cliente in datos:
    info_cliente = cliente.split("#")
    dni = int(info_cliente[0])
    nombre_apellido = info_cliente[1].split()
